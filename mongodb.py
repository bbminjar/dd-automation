...
def get_all_input_fields_and_add_to_data_dog(customer_name, api_key, app_key,support_mail, access_key, secret_key, slack_channels, customer_email_id, mongodb_connections_current, mongodb_mem_resident, mongodb_mem_virtual, alerts_list):
    ...
    if "mongodb" in alerts_list:
                    ...
                    api.Monitor.create(type="metric alert",
                               ...
                               options=options),
                    api.Monitor.create(type="metric alert",
                               query="min(last_30m):avg:mongodb.connections.current{*} by {host,name,region} >" + str(mongodb_connections_current),
                               ...
                               options=options),
                    api.Monitor.create(type="metric alert",
                               query="min(last_1h):avg:mongodb.connections.current{*} by {host,name,region} >" + str(mongodb_connections_current),
...
#Memory_Resident
                    ...
                    api.Monitor.create(type="metric alert",
                               ...
                               options=options),
                    api.Monitor.create(type="metric alert",
                               query="min(last_30m):avg:mongodb.mem.resident{*} by {host,name,region} >" + str(mongodb_mem_resident),
                               ...
                               options=options),
                    api.Monitor.create(type="metric alert",
                               query="min(last_1h):avg:mongodb.mem.resident{*} by {host,name,region} >" + str(mongodb_mem_resident),
...
#Memory_Resident
                    ...
                    api.Monitor.create(type="metric alert",
                               ...
                               options=options),
                    api.Monitor.create(type="metric alert",
                               query="min(last_30m):avg:mongodb.mem.virtual{*} by {host,name,region} >" + str(mongodb_mem_virtual),
                               name="[Escalation-TL]" + " - " + "["+str(customer_name)+"]" + " - " + "Mongodb resident memory is high",
                               message="Team,Mongodb virtual memory is high. Please take a look into this. @cloudops-business@minjar.com ",
                               tags=tags,
                               options=options),
                    api.Monitor.create(type="metric alert",
                               query="min(last_1h):avg:mongodb.mem.virtual{*} by {host,name,region} >" + str(mongodb_mem_virtual),
                               name="[Escalation]" + " - " + "["+str(customer_name)+"]" + " - " + "Mongodb resident memory is high",
                               message="Team,Mongodb virtual memory is high. Please take a look into this. @cloudops-business@minjar.com ",
                               tags=tags,
                               options=options),
