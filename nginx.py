...
def get_all_input_fields_and_add_to_data_dog(customer_name, api_key, app_key,support_mail, access_key, secret_key, slack_channels, customer_email_id, nginx_net_connections, alerts_list):
    ...
    if "nginx" in alerts_list:
            ...
            api.Monitor.create(type="metric alert",
                       ...
                       options=options),
            api.Monitor.create(type="metric alert",
                       query="min(last_30m):avg:nginx.net.connections{*} by {host,name,region} >" + str(nginx_net_connections),
                       ...
                       options=options),
            api.Monitor.create(type="metric alert",
                       query="min(last_1h):avg:nginx.net.connections{*} by {host,name,region} >" + str(nginx_net_connections),
