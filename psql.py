...
def get_all_input_fields_and_add_to_data_dog(customer_name, api_key, app_key,support_mail, access_key, secret_key, slack_channels, customer_email_id, postgresql_connections, postgresql_locks, alerts_list):
    ...
    if "psql" in alerts_list:
            ...
            api.Monitor.create(type="metric alert",
                       ...
                       options=options),
            api.Monitor.create(type="metric alert",
                       query="min(last_30m):avg:postgresql.connections{*} by {host,name,region} >" + str(postgresql_connections),
                       ...
                       options=options),
            api.Monitor.create(type="metric alert",
                       query="min(last_1h):avg:postgresql.connections{*} by {host,name,region} >" + str(postgresql_connections),
...
#locks
            ...
            api.Monitor.create(type="metric alert",
                       ...
                       options=options),
            api.Monitor.create(type="metric alert",
                       query="min(last_30m):avg:postgresql.locks{*} by {host,name,region} >" + str(postgresql_locks),
                       name="[Escalation-TL]" + " - " + "["+str(customer_name)+"]" + " - " + "JVM Thread count is high",
                       message="Team,JVM Thread count is high. Please take a look into this. @cloudops-business@minjar.com ",
                       tags=tags,
                       options=options),
            api.Monitor.create(type="metric alert",
                       query="min(last_1h):avg:postgresql.locks{*} by {host,name,region} >" + str(postgresql_locks),
                       name="[Escalation]" + " - " + "["+str(customer_name)+"]" + " - " + "JVM Thread count is high",
                       message="Team,JVM Thread count is high. Please take a look into this. @cloudops-business@minjar.com ",
                       tags=tags,
                       options=options),
